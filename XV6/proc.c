...
joinCid(int container_id){
  ...
  // A process must leave before joining
  if(currproc->in_container_id >0){
    // cprintf("=----------------------------------------------------------------------\n");
    return -1;
  }
...
int
con_malloc(int addr, int assign){

  struct proc * currproc = myproc();
  int container_id = currproc->in_container_id;
  int i ;
  acquire(&container_table.lock);
      ...
      {
        if (container_table.containers[i].cid==container_id){
            // gva_size = container_table.containers[i].mem_size;
            break; 
        }
      }
      ...
      if (mem_log==1){
        cprintf("%d -> %d\n",container_table.containers[i].mem_size,addr);
      }
      container_table.containers[i].mem_size += assign;
  ...
  return container_table.containers[i].mem_size;
}
